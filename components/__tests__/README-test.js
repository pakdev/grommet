...
var components = function components(folder) {
  return _fs['default'].readdirSync(folder).filter(function(file) {
    return (
      _fs['default']
        .statSync(_path['default'].join(folder, file))
        .isDirectory() &&
      _fs['default'].existsSync(_path['default'].join(folder, file, 'doc.js'))
    );
  });
};
...
test('README is updated', function(done) {
  ...
  componentFolders.sort().forEach(function(component) {
    readmeContent[component] = _fs['default'].readFileSync(
      _path['default'].join(FOLDER, component, 'README.md'),
      'utf8',
    );
  });
